class LandingPage < Edition
  ...
  validates :slug_override, presence: true, format: { with: /\A\/.*\z/, message: "must start with a slash (/)" }
...
private
  ...
  def slug_override_must_not_be_taken
    errors.add(:slug_override, " is already taken") if LandingPage.where(slug_override:).where.not(document_id: document_id).exists?
  end
end
